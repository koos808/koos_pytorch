...
* deque() 함수 : 제일 오래된 요소부터 없어지는 `큐(queue) 자료구조`
  * `self.memory = deque(maxlen=10000)`
  * deque의 maxlen을 지정하면 큐가 가득 찼을 때 제일 오래된 요소부터 없어지므로 자연스레 오래된 데이터는 삭제(잊게)된다.

* max(), min() : 최대값, 최소값이 해당하는 value(값)와 indices(위치)를 반환한다.
  * `torch.Tensor([1,3,2]).max(0)` & `torch.Tensor([1,3,2]).minx(0)`
  * max(0), max(1) -> max(0)이면 1차원에 있는 값, max(1)이면 2차원에 있는 최대값의 자리를 가져온다.

* gather() 함수
  * 첫 번째 인수로 받은 차원(1)에서 두 번째 인수로 받은 텐서(ex.actions)가 가리키는 값들을 추출한다.
  * ex 강화학습) 에이전트의 행동(Action) 64x1(1,0,1,...)이 있는데 이에 해당하는 모델의 반환값은 64x2([0.06, -0.02], [0.02, -0.14],..) 이다. gather()함수를 사용하면 모델의 반환값에서 Action에 해당하는 값을 가져온다. 즉, 64x1(-0.021 0.02,...)이 만들어 진다. 
